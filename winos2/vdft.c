...
LONG   VDHENTRY VDHRegisterAPI(PSZ,PFNHOOK,PFNHOOK);
...
BOOL HOOKENTRY VddDpmiEntry(ULONG ulMessageID,PVOID pvDataIn,PVOID pvDataOut);
...
BOOL HOOKENTRY VDFTCreate(HVDM hvdm)
{
    ...
    /*---------------------------------------------------------------*/
     ...
     if(!VDHRegisterAPI(VDDNAME,(PFNHOOK)0,(PFNHOOK)VddDpmiEntry))
     {
          return(FALSE);
     }
     return  TRUE;
