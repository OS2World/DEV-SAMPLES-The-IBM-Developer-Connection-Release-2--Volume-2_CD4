...
//
//-------------------------------------------------------------------------
public class LEDCell extends Panel
//public class LEDCell extends Canvas
{
  ...
  private static final int ELEM_TOP      = 0 ,
                           ELEM_TOPLEFT  = 1 ,
                           ELEM_TOPRIGHT = 2 ,
                           ELEM_BOTLEFT  = 3 ,
                           ELEM_BOTRIGHT = 4 ,
  ...
  //{
  //  System.out.println( "LEDCell::getPreferredSize() called" ) ;
  ...
  //{
  //  System.out.println( "LEDCell::getMinimumSize() called" ) ;
  //  return this.getSize() ;
  //}
  ...
  public void paint( Graphics g )
  {
    ...
    //System.out.println( "LEDCell::Paint() called." ) ;
  ...
  private void renderNumber( Graphics g )
  {
    ...
    polygonWidth = Math.max( 2, width/30 + height/30 ) ;
    ...
    polyY[0] = topY     ;
    polyX[1] = rightX   - polygonWidth   ;
    polyY[1] = topY     ;
    polyX[2] = polyX[1] - polygonWidth ; 
    polyY[2] = polyY[1] + polygonWidth ;
    polyX[3] = polyX[0] + polygonWidth ;
    polyY[3] = polyY[2] ;
    polyX[4] = polyX[0] ;
    polyY[4] = polyY[0] ;

    renderPolygon( g, polyX, polyY, 5, on[ELEM_TOP] ) ;
    ...
    renderPolygon( g, polyX, polyY, 5, on[ELEM_BOTTOM] ) ;
    ...
    renderPolygon( g, polyX, polyY, 5, on[ELEM_TOPLEFT] ) ;
    ...
    renderPolygon( g, polyX, polyY, 5, on[ELEM_TOPRIGHT] ) ;
    ...
    renderPolygon( g, polyX, polyY, 6, on[ELEM_MIDDLE] ) ;
    ...
    renderPolygon( g, polyX, polyY, 5, on[ELEM_BOTLEFT] ) ;
    ...
    renderPolygon( g, polyX, polyY, 5, on[ELEM_BOTRIGHT] ) ;
  ...
  //
  // Note that the value -1 is a special case that allows the middle
  // element only to be turned on. This is useful for special purpose
  // applications and is used by the LEDClockPanel class.
  //
  ...
  private void mapCharToLED( int value, boolean [] on )
  {
    ...
    if ( this.cellActive )
    {
      switch( value )
      {
        ...
        case 1:
        {
          on[ELEM_TOPRIGHT] = true ;
          on[ELEM_BOTRIGHT] = true ;
        }
        break ;
