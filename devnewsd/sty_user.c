 ...
 *          0 - if error
\****************************************************************/
BOOL DisPlayInComingMessage(PSZ pszNewMessage)
{
      static IPT iptOffset =0;

     strcat(pszNewMessage,"\r\n");
      /*
       * Set the file into the MLE
       */
      WinSendMsg(hwndMLE, MLM_SETIMPORTEXPORT, MPFROMP((PBYTE)pszNewMessage),
          MPFROMSHORT(strlen(pszNewMessage)+1 ));

      /*
       * Import to MLE starting at current pos
       */
