 ...
 void SayHelp( void )
 {
    ...
    printf("บ                                                                           บ\n");
    printf("บ ECI SAEOC002  - A.Thomson Summer 94 (See CCECIDOC for Documentation)      บ\n");
    printf("บ                                                                           บ\n");
    printf("บ C Program making call type ECI_STATE_ASYNC_SEM (with System semaphore)    บ\n");
    printf("บ                                                                           บ\n");
 ...
 void Make_One_Call_Set()
   {
     ...
     int SEMRetCode;
     HSEM  hsemEci;      /* Semaphore to wait on                */
 
     /***************************************************************************/
     /* Create the system semaphore to wait on (Note that the CSEM_PUBLIC flag  */
     /* is necessary. This thread creates the semaphore, while the ECI back-end */
     /* thread will clear it. The default flag is CSEM_PRIVATE, which means     */
     /* that OS/2 will not allow the ECI to clear the semaphore. The result is  */
     /* that an ECI application will hang on the DosSemWait() call if the       */
     /* default is used.):                                                      */
     /***************************************************************************/
     DosCreateSem ( CSEM_PUBLIC,
                    &hsemEci,
                    "\\SEM\\ECOAS16S_SEMAPHORE" ) ;

     /***************************************************************************/
     /* Set the semaphore:                                                      */
     /***************************************************************************/
     SEMRetCode = DosSemSet ( hsemEci ) ;
     if ( SEMRetCode != 0 )
       {
         printf ( "Could not reset semaphore. DosSemSet returned %d.\n",
                  SEMRetCode ) ;
       } /* endif */
    ...
    EciBlock.eci_call_type = ECI_STATE_ASYNC_SEM;    /* set call type */
    EciBlock.eci_extend_mode  = ECI_STATE_IMMEDIATE; /* set call type */
    EciBlock.eci_async_notify.sem_handle = hsemEci ; /* Semaphore     */
    ...
    if (ECIRetCode==ECI_NO_ERROR) 
      {
       ...
       /* Wait for CICS to clear semaphore */

       DosSemWait( hsemEci, SEM_INDEFINITE_WAIT );
 ...
 int main(argc, argv, envp)
    ...
    char *envp[];
   {
     ...
     printf("--------------------------------------------------------\n"); 
     printf(" ECI SAEOC0021 - Original Coding by A.Thomson Summer 94 \n"); 
     ...
     printf("--------------------------------------------------------\n"); 
     printf(" ECI SAEOC0021 - Application Termination\n"); 
     printf("--------------------------------------------------------\n"); 
