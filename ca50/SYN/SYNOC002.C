 ...
 printf("บ                                                                           บ\n");
 printf("บ ECI SYNOC002 - A.Thomson Summer 94 (See CCECIDOC for Documentation)       บ\n");
 printf("บ                                                                           บ\n");
 printf("บ C program making EXTENDED call type ECI_SYNC                              บ\n");
 printf("บ                                                                           บ\n");
 ...
 printf("บ                                                                           บ\n");
 printf("บ /L:luws         Number of concurrent Logical Units of Work                บ\n");
 printf("บ                                                                           บ\n");
 printf("บ /D:Delay        Value specifying time to wait between calls               บ\n");
 printf("บ                                                                           บ\n");
 ...
 void Make_One_Call_Set()
   {
     ...
     EciBlock.eci_call_type = ECI_SYNC; /* set call type */
     EciBlock.eci_extend_mode = ECI_EXTENDED; /* Extend Mode */
     EciBlock.eci_luw_token=0; /* set LUW to zero for initial call */
     ...
     for (Record=1; Record<=NUMRECS; Record++) /* For Each Record */
       {
         printf( "  Set of Calls %d ",Record );          

         switch(Record)
         {
         case 3        : EciBlock.eci_extend_mode=ECI_BACKOUT;
                         printf("(Extend_Mode = ECI_BACKOUT)\n");
                         break; 
         case NUMRECS : EciBlock.eci_extend_mode=ECI_COMMIT;
                         printf("(Extend_Mode = ECI_COMMIT)\n");
                         break; 
         default       : EciBlock.eci_extend_mode=ECI_EXTENDED;
                         printf("(Extend_Mode = ECI_EXTENDED)\n");
                         break; 
         }
         ...
         for (LUW=1; LUW<=Num_LUWS; LUW++) /* For each LUW */
           {            
             ...
             if (ECIRetCode!=ECI_NO_ERROR)
               ...
               {
                 printf( "   LUW=%d : ",LUW );
                 printf( "Extend_Mode=%d : ",EciBlock.eci_extend_mode );
                 printf( "RC=%d : ", ECIRetCode );
                 printf( "Token=%d : ", Tokens[LUW] );
                 CommArea[17] = '\0';
                 printf( "CA=%s\n", CommArea );
               }
 ...
 int main(argc, argv, envp)
   ...
   char *envp[];
   {
     ...
     memset(CommArea,'\0',COMMAREA_SIZE ); /* set Comm Area to Zeros */
     ...
     printf("--------------------------------------------------------\n"); 
     printf(" ECI SYNOC002 - Original Coding by A.Thomson Summer 94 \n"); 
    ...
    printf("--------------------------------------------------------\n"); 
    printf(" ECI SYNOC002 - Application Termination\n"); 
    printf("--------------------------------------------------------\n"); 
