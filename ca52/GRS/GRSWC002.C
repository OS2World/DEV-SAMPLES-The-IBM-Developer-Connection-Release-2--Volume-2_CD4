 ...
 {
 Info("                                                                            ");
 Info("  ECI GRSWC002 - A.Thomson Summer 94 (See CCECIDOC for Documentation)       ");
 Info("                                                                            ");
 Info("  C program making call type GET_SPECIFIC_REPLY_WAIT                        ");
 Info("                                                                            ");
 ...
 void Make_One_Call_Set(HWND hwnd)
   {
     ...
     if(ECIRetCode==ECI_NO_ERROR)
       { 
         ...
         /************************************************************************/
         sprintf ( szBuffer, "  ECI has returned. Perform GET_SPECIFIC_REPLY_WAIT" ) ;
         Info(szBuffer);
         Info("  Calling      : ECI_GET_SPECIFIC_REPLY_WAIT");
         /************************************************************************/
         /* Set up the ECI parameter block to solicit the status:               */
         /************************************************************************/
         EciBlock.eci_call_type = ECI_GET_SPECIFIC_REPLY_WAIT ;
         EciBlock.eci_commarea = &CommArea ;
         EciBlock.eci_commarea_length = sizeof ( CommArea ) ;
         /************************************************************************/
         /* Call the ECI:                                                       */
       ...
       {
         Info("  Initial call failed so no point in attempting to get reply");
       }
     Info(" -------------------------------------------------------------------------------- "); 
      ...
      case WM_COMMAND :
         switch ( wParam ) {
            ...
            case CALL_ECI       :
               ...
               Perform_Delay(Delay); 
               break;
               strcpy ( CommArea, "Windows sample ECI call." ) ;
            default :
               break ;
         } /* endswitch */
         break;
                      ...
                      int nCmdShow )
 {
   ...
   if ( hPrevInst == NULL ) {
    ...
    hwndFrame = CreateWindow ( ECI_CLASS,
                               "CICS Client Test - GRSWC002 (A.Thomson 94)",
                               WS_OVERLAPPEDWINDOW,
    ...
    Info(" -------------------------------------------------------------------------------- "); 
    Info(" ECI GRSWC002 - A.Thomson Summer 94 "); 
