 ...
 {
 Info("                                                                            ");
 Info("  ECI SSYWC001 - A.Thomson Summer 94 (See CCECIDOC for Documentation)      ");
 Info("                                                                            ");
 Info("  C program making call type ECI_STATE_SYNC                                 ");
 Info("                                                                            ");
    ...
    while ( j < argc )  {
       if ( strchr ( "/-(", argv[j][0] ) ) { /* looks like an option  */
          ...
          case 'S' :                         /* /S:Server             */
             ...
             sprintf(TempText,"Server Selected = %s",&argv[j][3]);
             Info(TempText);                                                         
             break;
          default :
 ...
 void Make_One_Call_Set()
   {
    ...
    EciBlock.eci_call_type = ECI_STATE_SYNC; /* set call type */
                      ...
                      int nCmdShow )
 {
   ...
   if ( hPrevInst == NULL ) {
    ...
    hwndFrame = CreateWindow ( ECI_CLASS,
                               "CICS Client Test - SSYWC001 (A.Thomson 94)",
                               WS_OVERLAPPEDWINDOW,
...
//  ParseParms( argc, argv );
    ...
    Info(" -------------------------------------------------------------------------------- "); 
    Info(" ECI SSYWC001 - A.Thomson Summer 94 "); 
