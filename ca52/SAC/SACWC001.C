 ...
 {
 Info("                                                                            ");
 Info("  ECI SACWC001 - A.Thomson Summer 94 (See CCECIDOC for Documentation)       ");
 Info("                                                                            ");
 Info("  C program making call type STATE_ASYNC with callback notification         ");
 Info("                                                                            ");
 ...
 void CICSEXIT CallBackProc (cics_ushort_t idx)
 {
   ...
   PostMessage (hwndMain, ECI_MSG, 0, NULL);
 ...
 void Make_One_Call_Set(HWND hwnd)
   {
     ...
     EciBlock.eci_call_type = ECI_STATE_ASYNC;
     ...
     CallBack = MakeProcInstance ((FARPROC)CallBackProc, hInstance);
     EciBlock.eci_callback = (CICS_EciNotify_t)CallBack;
     ...
     /*********************************************************************/
     Info(" -------------------------------------------------------------------------------- "); 
     Info("  Calling      : ECI_STATE_ASYNC with callback notify");
         ...
         if (ECIRetCode==ECI_NO_ERROR)
          {
           ...
           switch (EciStatus.CicsClientStatus)
            {
             case 0 : Info("  CicsClientStatus : ECI_CLIENTSTATE_UNKNOWN     "); break;
                      ...
                      int nCmdShow )
 {
   ...
   if ( hPrevInst == NULL ) {
    ...
    hwndFrame = CreateWindow ( ECI_CLASS,
                               "CICS Client Test - SACWC001 (A.Thomson 94)",
                               WS_OVERLAPPEDWINDOW,
    ...
    Info(" -------------------------------------------------------------------------------- "); 
    Info(" ECI SACWC001 - A.Thomson Summer 94 "); 
