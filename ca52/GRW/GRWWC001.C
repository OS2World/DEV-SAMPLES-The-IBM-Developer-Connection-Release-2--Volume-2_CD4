 ...
 Info("                                                                            ");
 Info("  C program making call type GET_REPLY_WAIT                                 ");
 Info("                                                                            ");
 ...
 void Make_One_Call_Set(HWND hwnd)
   {
     ...
     if(ECIRetCode==ECI_NO_ERROR)
       { 
         ...
         /************************************************************************/
         sprintf ( szBuffer, "  ECI has returned. Perform GET_REPLY_WAIT" ) ;
         Info(szBuffer);
                                  ...
                                  LONG     lParam )
 {
   ...
   switch ( msg ) {
      ...
      case WM_COMMAND :
         ...
         break;
      case ECI_MSG    :
         /* Do nothing but throw away the message ! */
         break;
      case WM_DESTROY :
         PostQuitMessage ( 0 ) ;
         break;
                      ...
                      int nCmdShow )
 {
   ...
   if ( hPrevInst == NULL ) {
    ...
    hwndFrame = CreateWindow ( ECI_CLASS,
                               "CICS Client Test - GRWWC001 (A.Thomson 94)",
                               WS_OVERLAPPEDWINDOW,
    ...
    Info(" -------------------------------------------------------------------------------- "); 
    Info(" ECI GRWWC001 - A.Thomson Summer 94 "); 
